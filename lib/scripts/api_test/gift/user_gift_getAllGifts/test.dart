...
const String API_NAME = '获取普通+VIP礼物列表';
const String API_PATH = '/user/gift/getAllGifts';
const String API_METHOD = 'GET';
const bool NEED_SIGN = false;

Future<Map<String, dynamic>> getAllGifts() async {
    ...
    if (result['code'] == 0) {
      ...
      if (list.isNotEmpty) {
        ...
        for (final gift in list) {
          ...
          final typeStr = giftType == 1 ? "普通礼物" : giftType == 2 ? "VIP礼物" : "未知";
          print('\n礼物 ${gift['gid']} - ${gift['name']} [$typeStr]:');
          print('  礼物ID: ${gift['gid']}');
          print('  名称: ${gift['name']}');
        ...
        // 统计各类礼物数量
        final normalCount = list.where((g) => g['giftType'] == 1).length;
        final vipCount = list.where((g) => g['giftType'] == 2).length;
        print('\n--- 礼物统计 ---');
        print('普通礼物: $normalCount 个');
        print('VIP礼物: $vipCount 个');
      } else {
        print('\n  无礼物数据');
      }
    } else {
