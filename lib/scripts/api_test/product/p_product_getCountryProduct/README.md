...
|------|------|
| 接口名称 | 按国家获取单个商品 |
| 接口路径 | `/p/product/getCountryProduct` |
| 请求方式 | GET |
| 需要签名 | 否 |
...
```
GET /p/product/getCountryProduct?productId=3&countryCode=840
```
...
| discountType | int | 折扣类型：1.首充折扣，2.单次折扣，3.限时折扣 |
...
| currencyPrice | int | 当地货币价格（单位：分） |
| currencyCode | string | 货币编码 |
| diamonds | int | 钻石数（小数点后两位，emma新版本才有） |
